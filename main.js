...
function prose(t){var ps=String(t||'').split('|');var h='';for(var i=0;i<ps.length;i++){var s=ps[i].trim();if(!s)continue;h+='<p>'+s.replace(/「([^」]+)」/g,'<span class="say">"$1"</span>').replace(/（([^）]+)）/g,'<span class="inner-thought">$1</span>').replace(/\(([^)]+)\)/g,'<span class="inner-thought">$1</span>')+'</p>';}return h;}
...
return'<div class="rp"><details><summary><div class="rp-h"><div class="rp-hl"><div class="dot dg"></div><span class="rp-ttl">个人面板</span></div><span class="rp-arr">▾</span></div></summary><div class="rp-bd"><div class="pf-top"><div class="pf-img" onclick="document.getElementById(\''+id+'\').click()">< img id="'+id+'-i" src="" style="display:none"><div id="'+id+'-p" class="pf-ph"><span class="pf-pht">点击<br>导入立绘</span></div></div><input type="file" accept="image/*" style="display:none" id="'+id+'" data-av="__USER__"><div class="pf-info"><div class="attr-rk">[ '+esc(rank)+' ] '+esc(title)+'</div>'+(comment?'<div class="attr-cm" style="font-size:10px;max-width:140px;line-height:1.4;word-break:break-all">'+esc(comment)+'</div>':'')+'</div></div><div style="height:1px;background:rgba(255,255,255,.05);margin:8px 0"></div>'+(equipH?'<div class="equip-section"><div class="pf-section-title">装备</div><div class="equip-zone">'+equipH+'</div></div>':'')+'<div class="pf-section-title">属性</div><div class="attr-grid">'+attrH+'</div></div></details></div>';}+(equipH?'<div class="equip-section"><div class="pf-section-title">装备</div><div class="equip-zone">'+equipH+'</div></div>':'')+'<div class="pf-section-title">属性</div><div class="attr-grid">'+attrH+'</div></div></details></div>';}
...
function gForum(data){var p=String(data||'').split('|');var au=p[0]||'',tag=p[1]||'',title=p[2]||'',body=p[3]||'',stats=p[4]||'0~0~0';var rawCmts=p.slice(5),st=stats.split('~');var topCmts=[],lastTop=null;for(var i=0;i<rawCmts.length;i++){var raw=rawCmts[i];if(raw.charAt(0)==='↳'){if(lastTop)lastTop.replies.push(raw.slice(1));}else{var c={raw:raw,replies:[]};topCmts.push(c);lastTop=c;}}var h='<div class="fp"><div class="fp-head">'+avHTML(au,'fp-av')+'<div class="fp-meta"><div class="fp-author">'+esc(au)+'</div><div class="fp-info-row"><span class="fp-tag">'+esc(tag)+'</span></div></div></div><div class="fp-title">'+esc(title)+'</div><div class="fp-body">'+esc(body)+'</div><div class="fp-footer"><span class="fp-stat">♡ '+esc(st[0]||'0')+(st[3]?' <span style="font-size:.85em;color:var(--ts);margin-left:2px">'+esc(st[3])+'</span>':'')+'</span><span class="fp-stat">✦ '+esc(st[2]||'0')+'</span><span class="fp-stat">↻ '+esc(st[1]||'0')+'</span></div>';if(topCmts.length){h+='<details class="fp-cmt-details" open><summary>共 '+topCmts.length+' 条评论 ▾</summary><div class="fp-cmt-area">';for(var i=0;i<topCmts.length;i++){var tc=topCmts[i],ci2=tc.raw.indexOf(':');if(ci2<0)ci2=tc.raw.indexOf(':');if(ci2<0)continue;var ca=tc.raw.slice(0,ci2).trim(),ctxt=tc.raw.slice(ci2+1).trim(),likes=hashLikes(tc.raw);h+='<div class="fp-cmt-block"><div class="fp-cmt-i">'+avHTML(ca,'fp-cmt-av')+'<div class="fp-cmt-ct"><div class="fp-cmt-hr"><span class="fp-cmt-au">'+esc(ca)+'</span></div><div class="fp-cmt-tx">'+esc(ctxt)+'</div><div class="fp-cmt-foot"><span class="cmt-act cmt-like">♡ '+likes+'</span></div></div></div>';if(tc.replies.length){h+='<div class="fp-cmt-replies">';for(var j=0;j<tc.replies.length;j++){var rr=tc.replies[j],ri=rr.indexOf(':');if(ri<0)ri=rr.indexOf(':');if(ri<0)continue;var ra=rr.slice(0,ri).trim(),rt=rr.slice(ri+1).trim(),rl=hashLikes(rr);h+='<div class="fp-cmt-reply">'+avHTML(ra,'fp-cmt-rav')+'<div class="fp-cmt-rct"><div class="fp-cmt-rhr"><span class="fp-cmt-rau">'+esc(ra)+'</span></div><div class="fp-cmt-rtx">'+esc(rt)+'</div><div class="fp-cmt-rfoot"><span class="cmt-act cmt-like">♡ '+rl+'</span></div></div></div>';}h+='</div>';}h+='</div>';}h+='</div></details>';}return h+'</div>';}

function gTouch(data){
var p=String(data||'').split('|');
var charName=p[0]||'';var charMood=p[1]||'50';var charOs=p[2]||'';var partsRaw=p.slice(3);
var moodNum=parseInt(charMood)||50;var moodPct=Math.max(0,Math.min(100,moodNum));
var moodCol=moodPct>=70?'rgba(225,160,170,.9)':moodPct>=50?'rgba(210,145,138,.8)':moodPct>=35?'rgba(180,160,200,.8)':'rgba(130,170,210,.8)';
var h='<div class="tp"><div class="tp-hb"><svg viewBox="0 0 400 80" preserveAspectRatio="none"><polyline points="0,40 30,40 42,15 54,65 66,25 78,55 90,35 110,40 140,40 152,12 164,68 176,22 188,58 200,38 220,40 250,40 262,18 274,62 286,28 298,52 310,40 340,40 352,15 364,65 376,30 388,50 400,40" fill="none" stroke="rgba(210,145,138,.6)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="tp-inner"><div class="tp-top">';
h+='<div class="tp-av">'+esc(charName.charAt(0))+'</div>';
h+='<div class="tp-name">'+esc(charName)+'</div>';
h+='<div class="tp-mood"><div class="tp-mood-bar"><span class="tp-mood-label">心情</span><div class="tp-mood-track"><div class="tp-mood-fill" style="width:'+moodPct+'%;background:'+moodCol+'"></div></div><span class="tp-mood-num" style="color:'+moodCol+'">'+moodPct+'</span></div></div>';
h+='<div class="tp-os">'+esc(charOs)+'</div></div>';
h+='<div class="tp-grid">';
for(var i=0;i<partsRaw.length;i++){
var sp=partsRaw[i].split('~');var partName=sp[0]||'';var partIcon=sp[1]||'◈';var partReaction=sp[2]||'';var partInner=sp[3]||'';var partDelta=sp[4]||'0';
var deltaNum=parseInt(partDelta)||0;var deltaText=deltaNum>0?'♡ +'+deltaNum:deltaNum<0?'♡ '+deltaNum:'♡ ±0';var deltaCls=deltaNum>0?'up':deltaNum<0?'down':'zero';
h+='<div class="tp-btn" onclick="tpClick(this,\''+esc(partIcon)+'\',\''+esc(partName)+'\',\''+deltaCls+'\',\''+esc(deltaText)+'\',this.getAttribute(\'data-r\'),this.getAttribute(\'data-i\'))" data-r="'+esc(partReaction)+'" data-i="'+esc(partInner)+'">';
h+='<span class="tp-btn-icon">'+esc(partIcon)+'</span><span class="tp-btn-name">'+esc(partName)+'</span></div>';
}
h+='</div><div class="tp-hint">点击部位查看角色反应 · 每个部位仅可触摸一次</div></div></div>';
return h;
}

function gPyq(data){
var p=String(data||'').split('|');
var author=p[0]||'';var text=p[1]||'';var time=p[2]||'';var likes=p[3]||'';var cmtsRaw=p.slice(4);
var h='<div class="pyq"><div class="pyq-hd"><div class="pyq-av">'+esc(author.charAt(0))+'</div><div class="pyq-meta"><div class="pyq-name">'+esc(author)+'</div><div class="pyq-text">'+esc(text)+'</div></div></div>';
h+='<div class="pyq-ft"><div class="pyq-time">'+esc(time)+'</div>';
if(likes){h+='<div class="pyq-likes-area"><span class="pyq-likes-icon">♥</span><span class="pyq-likes-names">'+esc(likes)+'</span></div>';}
h+='</div>';
if(cmtsRaw.length){
h+='<div class="pyq-divider"></div><div class="pyq-cmts">';
for(var i=0;i<cmtsRaw.length;i++){
var c=cmtsRaw[i];var replyIdx=c.indexOf('→');
if(replyIdx>0){var ca=c.slice(0,replyIdx).trim();var rest=c.slice(replyIdx+1).trim();var ci2=rest.indexOf(':');if(ci2<0)ci2=rest.indexOf(':');if(ci2>0){h+='<div class="pyq-cmt"><span class="pyq-cmt-name">'+esc(ca)+'</span><span class="pyq-cmt-reply">回复 '+esc(rest.slice(0,ci2).trim())+'：</span><span class="pyq-cmt-text">'+esc(rest.slice(ci2+1).trim())+'</span></div>';}else{h+='<div class="pyq-cmt"><span class="pyq-cmt-name">'+esc(ca)+'</span><span class="pyq-cmt-text">'+esc(rest)+'</span></div>';}}
else{var ci3=c.indexOf(':');if(ci3<0)ci3=c.indexOf(':');if(ci3>0){h+='<div class="pyq-cmt"><span class="pyq-cmt-name">'+esc(c.slice(0,ci3).trim())+'：</span><span class="pyq-cmt-text">'+esc(c.slice(ci3+1).trim())+'</span></div>';}}
}h+='</div>';}
h+='</div>';return h;
}
...
else if(b.t==='消息'){if(b.d.indexOf('NSFW')===0)nsfw=b.d.replace(/^NSFW[\|~]/,'');else{var mm=b.d.split(/[|,]/);for(var mi=0;mi<mm.length;mi++){if(mm[mi].trim()&&mm[mi].indexOf('~')>0)msgs.push(mm[mi].trim());}}}
else if(b.t==='直播'){if(b.d==='关')lOff=true;else{var parts2=b.d.split('|');for(var j=0;j<parts2.length;j++){if(parts2[j].indexOf('观众~')===0)lV=parts2[j].split('~')[1]||'0';else if(parts2[j].indexOf('打赏~')===0)lT=parts2[j].slice(3);else lB.push(parts2[j]);}}}
...
else if(b.t==='触摸')html+=gTouch(b.d);
else if(b.t==='朋友圈')html+=gPyq(b.d);
else if(b.t==='文风'){LS.setItem('wre_style_active',b.d);}
else if(b.t==='通讯录'){var cl=b.d.split('|');cl.forEach(function(c){var pp=c.split('~');if(pp[0]){var cd2=JSON.parse(LS.getItem('wre_contacts')||'{"groups":{"默认":[]}}');var exists2=false;Object.keys(cd2.groups).forEach(function(g){cd2.groups[g].forEach(function(x){if(x.name===pp[0])exists2=true;});});if(!exists2){cd2.groups['默认'].push({name:pp[0],desc:pp[1]||'',yaml:'name: '+pp[0]+'\ndesc: '+(pp[1]||'')});LS.setItem('wre_contacts',JSON.stringify(cd2));}}});}
else if(b.t==='成就'){var al=b.d.split('|');al.forEach(function(a){var pp=a.split('~');if(pp[0]){var achs=JSON.parse(LS.getItem('wre_achv')||'[]');var exists3=achs.some(function(x){return x.name===pp[0]});if(!exists3){achs.push({badge:pp[2]||'🏆',name:pp[0],desc:pp[1]||'',time:time.m+' '+time.h});LS.setItem('wre_achv',JSON.stringify(achs));}}});}
...
else if(b.t==='记账'){try{var lg=JSON.parse(LS.getItem('wre_ledger')||'[]');var ops2=b.d.split(',');ops2.forEach(function(op){var pp=op.split('~');lg.push({type:pp[0]==='+'?'inc':'exp',name:pp[1]||'',time:time.h,amt:(pp[0]==='+'?'+':'-')+(pp[2]||'')});});LS.setItem('wre_ledger',JSON.stringify(lg));}catch(x){}}
}
...
R.innerHTML=html;
var bl=LS.getItem('wre_blur');if(bl)document.documentElement.style.setProperty('--gb','blur('+bl+'px) saturate(140%)');
...
window.tpClick=function(btn,icon,part,deltaCls,deltaText,reaction,inner){btn.classList.add('touched');btn.style.pointerEvents='none';btn.style.opacity='.4';document.getElementById('popIcon').textContent=icon;document.getElementById('popPart').textContent=part;var de=document.getElementById('popDelta');de.textContent=deltaText;de.className='tp-pop-delta '+deltaCls;document.getElementById('popReaction').textContent=reaction;document.getElementById('popInner').textContent=inner;document.getElementById('tpPopup').classList.add('show');};
...
window.togglePhone=function(){var pw=document.getElementById('phoneWrap');pw.classList.toggle('open');if(pw.classList.contains('open'))posPhone();};
function posPhone(){var pw=document.getElementById('phoneWrap');var orb=document.getElementById('orbWrap');if(!pw||!orb)return;var ox=orb.offsetLeft||0;var oy=orb.offsetTop||0;if(!orb.style.left){ox=window.innerWidth-64;oy=window.innerHeight-116;}var pw2=272,ph=440;var left=ox-pw2/2+23;if(left+pw2>window.innerWidth)left=window.innerWidth-pw2-8;if(left<4)left=4;var top=oy-ph-12;if(top<4)top=oy+56;pw.style.right='auto';pw.style.bottom='auto';pw.style.left=left+'px';pw.style.top=top+'px';}
...
if(saved){var svgs=JSON.parse(LS.getItem('wre_icon_svg')||'null');if(svgs)document.getElementById('orbBtn').innerHTML=svgs;}
})();
(function(){var orb=document.getElementById('orbWrap'),ox=0,oy=0,mx=0,my=0,dragging=false;if(!orb)return;var sp=LS.getItem('wre_orbpos');if(sp){try{var pp=JSON.parse(sp);orb.style.right='auto';orb.style.bottom='auto';orb.style.left=pp.x+'px';orb.style.top=pp.y+'px';}catch(e){}}orb.addEventListener('touchstart',function(e){var t=e.touches[0];ox=orb.offsetLeft;oy=orb.offsetTop;mx=t.clientX;my=t.clientY;dragging=false;},{passive:true});orb.addEventListener('touchmove',function(e){dragging=true;var t=e.touches[0];var nx=ox+(t.clientX-mx),ny=oy+(t.clientY-my);orb.style.right='auto';orb.style.bottom='auto';orb.style.left=Math.max(0,Math.min(window.innerWidth-50,nx))+'px';orb.style.top=Math.max(0,Math.min(window.innerHeight-50,ny))+'px';posPhone();e.preventDefault();},{passive:false});orb.addEventListener('touchend',function(){if(dragging){LS.setItem('wre_orbpos',JSON.stringify({x:orb.offsetLeft,y:orb.offsetTop}));dragging=false;}});orb.addEventListener('mousedown',function(e){ox=orb.offsetLeft;oy=orb.offsetTop;mx=e.clientX;my=e.clientY;dragging=false;var onmove=function(ev){dragging=true;var nx=ox+(ev.clientX-mx),ny=oy+(ev.clientY-my);orb.style.right='auto';orb.style.bottom='auto';orb.style.left=Math.max(0,Math.min(window.innerWidth-50,nx))+'px';orb.style.top=Math.max(0,Math.min(window.innerHeight-50,ny))+'px';posPhone();};var onup=function(){document.removeEventListener('mousemove',onmove);document.removeEventListener('mouseup',onup);if(dragging){LS.setItem('wre_orbpos',JSON.stringify({x:orb.offsetLeft,y:orb.offsetTop}));}};document.addEventListener('mousemove',onmove);document.addEventListener('mouseup',onup);});})();
