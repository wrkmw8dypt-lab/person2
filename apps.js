...
function updPreview(){var c=LS.getItem('wre_color')||'#c8c0b2';var pt=document.getElementById('prevT');if(pt)pt.style.color=c;}
...
var sv=sel[slot]||'';var cv=col[slot]||'';var display=cv?(cv+sv):sv;
h+='<div style="display:flex;align-items:center;gap:5px;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.04)"><span style="font-size:.55em;color:var(--td);width:28px;flex-shrink:0;text-align:right">'+slot+'</span><span style="padding:2px 5px;border-radius:4px;border:1px solid rgba(255,255,255,.08);font-size:.52em;color:var(--ts);cursor:pointer;min-width:26px;text-align:center" onclick="rollWdCol(\''+slot+'\')">'+esc(cv||'色')+'</span><span style="font-size:.68em;cursor:pointer;width:18px;text-align:center" onclick="rollWdName(\''+slot+'\')">🎲</span><span style="flex:1;font-size:.58em;color:'+(display?'var(--tp)':'var(--td)')+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(display||'点🎲随机')+'</span></div>';
});
h+='</div>';
...
(q?items:data.groups[g]).forEach(function(c,i){var idx=data.groups[g].indexOf(c);
...
{cmd:'【指令】',name:'万能触发',desc:'全角括号包裹触发功能'},
...
var cur=LS.getItem('wre_style')||'淡水幸福';
var body=document.getElementById('styleBody');
body.innerHTML='<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:10px;text-align:center;margin-bottom:8px"><div style="font-size:.48em;color:var(--td);letter-spacing:2px;margin-bottom:3px">当前启用</div><div style="font-size:.95em;color:var(--tp);font-weight:500">'+cur+'</div></div><div style="display:flex;flex-direction:column;gap:3px">'+styles.map(function(s){return'<div style="padding:7px 9px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:7px;cursor:pointer" onclick="pickStyle(\''+s.n+'\')"><div style="font-size:.68em;color:var(--tp)">'+s.n+(s.n===cur?' ✓':'')+'</div><div style="font-size:.5em;color:var(--td);margin-top:1px">'+s.d+'</div></div>';}).join('')+'</div>';
};
